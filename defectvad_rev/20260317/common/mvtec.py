...
def get_transform(split, img_size=256, crop_size=None, normalize=True):
    transform = [
        ...
        T.Resize((img_size, img_size)), # image: bilinear / mask: nearest
    ]
    ...
    if split == "train":
        transform.extend([
            ...
            # T.RandomChoice([
            #     T.Identity(),
            #     T.Affine(degrees=90, interpolation=T.InterpolationMode.BILINEAR, fill=0),
            #     T.Affine(degrees=180, interpolation=T.InterpolationMode.BILINEAR, fill=0),
            #     T.Affine(degrees=270, interpolation=T.InterpolationMode.BILINEAR, fill=0),
            # ]),
        ])
    if normalize:
...
if __name__ == "__main__":
    ...
    print(f"Mask: {masks.shape}, [{masks.min()}, {masks.max()}]")
