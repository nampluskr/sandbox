...
from defectvad.models.efficientad.torch_model import EfficientAdModel, EfficientAdModelSize, reduce_tensor_elems
...
class EfficientAdTrainer(BaseTrainer):
    def __init__(self, model, loss_fn=None, device=None, evaluator=None, model_size="small"):
    ...
    def prepare_imagenette_data(self, image_size: tuple[int, int] | torch.Size) -> None:
        ...
        self.imagenet_loader = DataLoader(imagenet_dataset, batch_size=self.batch_size, shuffle=True)
        self.imagenet_iterator = iter(self.imagenet_loader)
...
if __name__ == "__main__":
    ...
    EXPERIMENT_NAME = "mvtec_03_efficientad"
    ...
    trainer = EfficientAdTrainer(model, model_size="small", evaluator=Evaluator)
    trainer.fit(train_loader, max_epochs=3, valid_loader=test_loader)
    ...
    #         pad_maps=True,
    #     )
    # trainer = EfficientAdTrainer(model, model_size="medium", evaluator=Evaluator)
    # trainer.fit(train_loader, max_epochs=3, valid_loader=test_loader)
