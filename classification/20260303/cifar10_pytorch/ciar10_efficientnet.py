...
if __name__ == "__main__":
    ...
    from torchvision.models import (efficientnet_b0, efficientnet_b1, efficientnet_b2,
        efficientnet_b3, efficientnet_b5, efficientnet_b7)

    BACKBONE_DIR = r"D:\Non_Documents\backbones"
    model_dict = {
    ...
    weight_path_dict = {
        "efficientnet_b0": os.path.join(BACKBONE_DIR, "efficientnet_b0_rwightman-7f5810bc.pth"),
        "efficientnet_b1": os.path.join(BACKBONE_DIR, "efficientnet_b1-c27df63c.pth"),
        "efficientnet_b2": os.path.join(BACKBONE_DIR, "efficientnet_b2_rwightman-c35c1473.pth"),
        "efficientnet_b3": os.path.join(BACKBONE_DIR, "efficientnet_b3_rwightman-b3899882.pth"),
        "efficientnet_b5": os.path.join(BACKBONE_DIR, "efficientnet_b5_lukemelas-1a07897c.pth"),
        "efficientnet_b7": os.path.join(BACKBONE_DIR, "efficientnet_b7_lukemelas-c5b4e57e.pth"),
    }
    feature_dim_dict = {
    ...
    class EfficientNetClassifier(nn.Module):
        def __init__(self, model_name="efficientnet_b0", num_classes=10, pretrained=True):
            ...
            if pretrained:
                state_dict = torch.load(weight_path_dict[model_name], map_location='cpu')
                self.backbone.load_state_dict(state_dict, strict=False)

            self.backbone.classifier = nn.Sequential(
                nn.Dropout(0.2),
                nn.Linear(feature_dim_dict[model_name], num_classes)
            )

        def forward(self, x):
            return self.backbone(x)

    model = EfficientNetClassifier("efficientnet_b7", num_classes=10, pretrained=True)
