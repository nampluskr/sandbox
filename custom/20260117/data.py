...
class MeasuredData(BaseData):
    ...
    def split(self, std=[], unit=None):
        ...
        for i in range(len(std)):
            ...
            axes[i].set_title(f"Y ($\\sigma$={std[i]:.2f})")
    ...
    def hist(self, std=[], unit=None):
        ...
        axes[2].hist(self.calibrate(mu=0.5, shift=True).ravel(), bins=256, range=(0, 1), alpha=0.6, density=True)
        ...
        for i in range(len(std)):
            ...
            axes[3 + i].hist(gray.ravel(), bins=256, range=(0, 1), alpha=0.6, density=True)
            axes[3 + i].set_title(f"Y ($\\sigma$={std[i]:.2f})")
